...
router.get("/habitacion/:habitacionId", (req: Request, res: Response) => {
  try {
    ...
    const habitacion = db
      ...
      .get(habitacionId) as Habitacion | undefined;
...
router.post("/", (req: Request, res: Response) => {
  try {
    ...
    if (![0, 1, 2].includes(estado)) {
      return res.status(400).json({
        error:
          "Estado inválido. Valores permitidos: 0 (libre), 1 (reservado), 2 (ocupado)",
      });
    }
    ...
    const registroExiste = db
      .query("SELECT * FROM registros WHERE habitacion_id = ?")
      .get(habitacion_id);
    if (registroExiste) {
      return res.status(400).json({
        error:
          "Ya existe un registro para esta habitación. Use PUT para actualizar.",
      });
    }
    ...
    const query = db.query(
      "INSERT INTO registros (habitacion_id, usuario_id, estado, fecha_registro) VALUES (?, ?, ?, ?) RETURNING *",
    );
    const registro = query.get(
      habitacion_id,
      usuario_id || null,
      estado,
      fecha_registro,
...
router.put("/:id", (req: Request, res: Response) => {
  try {
    ...
    const fecha_registro = new Date().toISOString();
    const query = db.query(
      "UPDATE registros SET usuario_id = ?, estado = ?, fecha_registro = ? WHERE id = ? RETURNING *",
    );
